...
Imagine we set a breakpoint on line 37. The values of variables is quite interesting!
...
The first thing we encounter on line 7 is an import for `moduleC.js`. 
...
This triggers another call to `__webpack_require__`, 
just like the first line of `moduleC` which triggers yet another call to `__webpack_require__` to resolve `moduleB`.
...
As we can see now, the value of 'const bar' on line 15 will become `undefined + 'bar'` and we can also set the default export to `"undefinedbar"`
